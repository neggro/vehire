...
export default async function HostProfilePage({ params }: HostPageProps) {
  ...
  const memberDuration = memberMonths < 12
    ? `${memberMonths} meses`
    : `${Math.floor(memberMonths / 12)} año${Math.floor(memberMonths / 12) > 1 ? "s" : ""}`;
            ...
            <div className="flex-1 text-center sm:text-left space-y-2">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4 text-sm text-muted-foreground">
                <span className="flex items-center gap-1">
                  ...
                  Miembro desde {memberYear} ({memberDuration})
                </span>
                {host.kycVerified && (
                  <span className="flex items-center gap-1">
          ...
          <Card>
            <CardContent className="p-6">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-6">
                <StatCard icon={Car} value={host.vehicles.length} label="Vehículos activos" />
                <StatCard icon={Users} value={host.completedTrips} label="Viajes completados" />
                <StatCard
                  icon={Star}
                  value={host.rating ? host.rating.toFixed(1) : "—"}
                  label="Calificación promedio"
                />
                <StatCard icon={MessageCircle} value={`${host.responseRate}%`} label="Tasa de respuesta" />
              </div>
            </CardContent>
          </Card>
          ...
          {host.vehicles.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold font-display">
                Vehículos de {host.fullName.split(" ")[0]}
              </h2>
          ...
          {host.reviewCount > 0 ? (
            ...
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <h3 className="font-semibold mb-1">Sin reseñas aún</h3>
                <p className="text-sm text-muted-foreground">
                  {host.fullName.split(" ")[0]} aún no ha recibido reseñas
                </p>
              </CardContent>
            </Card>
          )}
