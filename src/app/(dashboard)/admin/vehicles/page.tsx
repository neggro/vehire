...
export default function AdminVehiclesPage() {
  ...
  const handleQuickAction = async (vehicleId: string, action: "ACTIVE" | "REJECTED") => {
    try {
      ...
      if (res.ok) {
        toast({ title: action === "ACTIVE" ? "Vehiculo aprobado" : "Vehiculo rechazado" });
        fetchVehicles();
      } else {
  ...
  return (
    <div className="container py-8">
      <div className="mb-6">
        <h1 className="text-3xl font-bold">Gestion de Vehiculos</h1>
        <p className="text-muted-foreground">
          {data ? `${data.total} vehiculos registrados` : "Cargando..."}
        </p>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left px-4 py-3 font-medium hidden md:table-cell">Anfitrion</th>
                  <th className="text-left px-4 py-3 font-medium hidden lg:table-cell">Ciudad</th>
                  <th className="text-right px-4 py-3 font-medium hidden sm:table-cell">Precio/dia</th>
                      ...
                      <td className="px-4 py-3 text-right">
                        {vehicle.status === "PENDING_APPROVAL" ? (
                          ...
                          <Button variant="ghost" size="sm" asChild>
                            <Link href={`/admin/vehicles/${vehicle.id}`}>Ver</Link>
                          </Button>
                        )}
                      </td>
                    </tr>
