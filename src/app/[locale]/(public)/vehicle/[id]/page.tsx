...
async function getVehicle(id: string): Promise<VehicleData | null> {
  ...
  // Rules will be built with translations in the component
  const rules: string[] = [];
...
async function VehicleInfo({ vehicle }: { vehicle: VehicleData }) {
  const t = await getTranslations("vehicle.detail");
  ...
  const rules = [
    ...
    t("noPets"),
    t("returnFuel"),
    vehicle.mileageLimit ? t("maxKmPerDay", { limit: vehicle.mileageLimit }) : null,
    ...
    <div className="space-y-6">
          ...
          {vehicle.rating && (
            <>
              <div className="flex items-center gap-1">
                ...
                <span className="text-muted-foreground">{t("reviewCount", { count: vehicle.reviewCount })}</span>
              </div>
      ...
      <div className="flex flex-wrap gap-4">
        <Badge variant="secondary" className="gap-1 px-3 py-1">
          ...
          {vehicle.seats} {tc("labels.seats")}
        </Badge>
        <Badge variant="secondary" className="gap-1 px-3 py-1">
          <Gauge className="h-4 w-4" />
          {tc(`transmission.${vehicle.transmission.toLowerCase()}`) || vehicle.transmission}
        </Badge>
        <Badge variant="secondary" className="gap-1 px-3 py-1">
          <Fuel className="h-4 w-4" />
          {tc(`fuelType.${vehicle.fuelType.toLowerCase()}`) || vehicle.fuelType}
        </Badge>
        <Badge variant="secondary" className="gap-1 px-3 py-1">
          {vehicle.color}
        </Badge>
      ...
      {vehicle.description && (
        <div>
          <h2 className="mb-2 text-xl font-semibold">{t("description")}</h2>
      ...
      {vehicle.features.length > 0 && (
        <div>
          <h2 className="mb-4 text-xl font-semibold">{t("features")}</h2>
            ...
            {vehicle.features.map((featureId) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm">{tc(`features.${featureId}`) || feature.label}</span>
                </div>
              );
            })}
          </div>
      ...
      {/* Rules */}
      <div>
        <h2 className="mb-4 text-xl font-semibold">{t("rules")}</h2>
      ...
      {vehicle.mileageLimit && (
        <div className="rounded-lg bg-muted p-4">
          <p className="text-sm text-muted-foreground">
            <strong>{t("mileage")}</strong> {t("mileageDetail", { limit: vehicle.mileageLimit })}
          </p>
        </div>
      )}
    </div>
...
async function HostCard({ host }: { host: VehicleData["host"] }) {
  const t = await getTranslations("vehicle.detail");
  const tc = await getTranslations("common");
      ...
      <CardContent className="p-6">
          ...
          <div className="flex-1">
            ...
            {host.rating && (
              <div className="mt-1 flex items-center gap-1 text-sm text-muted-foreground">
                ...
                <span>{t("reviewCount", { count: host.reviewCount })}</span>
              </div>
            )}
            <div className="mt-2 text-sm text-muted-foreground">
              <p>{host.tripsCount} {tc("labels.activeVehicles")}</p>
              <p>{tc("labels.memberSince", { year: new Date(host.memberSince).getFullYear() })}</p>
            </div>
          </div>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-2">
          ...
          <Button variant="outline" className="w-full" asChild>
            <Link href={`/host/${host.id}`}>{t("viewProfile")}</Link>
          </Button>
        </div>
      </CardContent>
    </Card>
...
async function LocationMap({ vehicle }: { vehicle: VehicleData }) {
  const t = await getTranslations("vehicle.detail");
