...
function BookingContent() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.pendingReservation) {
            ...
            toast({
              title: t("reservationRecovered"),
              description: t("reservationRecoveredDesc"),
            });
          }
        }
      } catch (error) {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 404) {
            ...
            setError(t("errorLoadingVehicle"));
          }
          return;
        }
        const data = await response.json();
  ...
  const createPendingReservation = async (): Promise<{ id: string } | null> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || t("errorCreatingBooking"));
      }
      ...
      toast({
        title: tc("labels.error"),
        description: err instanceof Error ? err.message : t("errorCreatingBooking"),
        variant: "destructive",
      });
      return null;
    } finally {
  ...
  const handleCardPayment = async (paymentData: {
    ...
    try {
      ...
      toast({
        ...
        description: err instanceof Error ? err.message : t("errorProcessingPayment"),
        variant: "destructive",
      });
    }
  };
  ...
  const createPayPalOrder = async (): Promise<string> => {
    ...
    if (!currentReservation) {
      const reservation = await createPendingReservation();
      if (!reservation) {
        throw new Error(t("errorCreatingBooking"));
      }
      currentReservation = { id: reservation.id, status: "PENDING", totalAmount: calculation?.totalAmount || 0 };
    }
