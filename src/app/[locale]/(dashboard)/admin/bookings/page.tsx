    ...
    <div className="container py-8">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          {data ? t("bookingsTotal", { count: data.total }) : tc("actions.loading")}
        </p>
      </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="PENDING">{tc("bookingStatus.PENDING")}</SelectItem>
                  <SelectItem value="CONFIRMED">{tc("bookingStatus.CONFIRMED")}</SelectItem>
                  <SelectItem value="ACTIVE">{tc("bookingStatus.ACTIVE")}</SelectItem>
                  <SelectItem value="COMPLETED">{tc("bookingStatus.COMPLETED")}</SelectItem>
                  <SelectItem value="CANCELLED">{tc("bookingStatus.CANCELLED")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="flex flex-col sm:flex-row gap-4">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left px-4 py-3 font-medium">{t("vehicle")}</th>
                  <th className="text-left px-4 py-3 font-medium hidden md:table-cell">{t("driver")}</th>
                  <th className="text-left px-4 py-3 font-medium hidden lg:table-cell">{t("host")}</th>
                  <th className="text-left px-4 py-3 font-medium hidden sm:table-cell">{t("dates")}</th>
                  <th className="text-right px-4 py-3 font-medium">{t("total")}</th>
                  <th className="text-left px-4 py-3 font-medium">{t("status")}</th>
                  <th className="text-left px-4 py-3 font-medium hidden xl:table-cell">{t("payment")}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={booking.id} className="border-b hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3 text-muted-foreground hidden sm:table-cell">
                      <span className="whitespace-nowrap">
                        {new Date(booking.startDate).toLocaleDateString(locale, { day: "2-digit", month: "short" })}
                        {" - "}
                        {new Date(booking.endDate).toLocaleDateString(locale, { day: "2-digit", month: "short" })}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 hidden xl:table-cell">
                      {booking.payment ? (
                        ...
                        <span className="text-xs text-muted-foreground">{t("noPayment")}</span>
                      )}
                    </td>
                  </tr>
                ))}
