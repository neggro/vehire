  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({ title: tc("labels.error"), description: err.error || t("updateError"), variant: "destructive" });
      }
    } finally {
      setSaving(false);
    }
      ...
      <div className="grid gap-6 lg:grid-cols-3">
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center text-center">
              ...
              <Badge variant={kycBadgeVariant[user.kycStatus]} className="mt-2">
                KYC: {tc(`labels.${user.kycStatus.toLowerCase()}`)}
              </Badge>
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{t("bookingsAsDriver")}</p>
              </div>
              <div>
                <div className="text-2xl font-bold">{user._count.bookingsAsHost}</div>
                <p className="text-xs text-muted-foreground">{t("bookingsAsHost")}</p>
              </div>
              <div>
                <div className="text-2xl font-bold">{user._count.vehicles}</div>
                <p className="text-xs text-muted-foreground">{t("vehiclesCount")}</p>
              </div>
              <div>
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>{t("rolesPermissionsKyc")}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Roles */}
            <div>
              <Label className="text-sm font-medium mb-3 block">{t("roles")}</Label>
            ...
            {roles.includes("ADMIN") && (
              <div>
                ...
                <Label className="text-sm font-medium mb-3 block">{t("adminPermissions")}</Label>
            ...
            {/* KYC Status */}
            <div>
              <Label className="text-sm font-medium mb-2 block">{t("kycStatus")}</Label>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b bg-muted/50">
                      ...
                      <th className="text-left px-4 py-3 font-medium">{t("dates")}</th>
